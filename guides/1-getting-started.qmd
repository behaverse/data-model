...
- The `data/<AGENT>/` folder contains the data for a single human subject or computer agent. It is organize by session folders for each session of the study. It may also contain other agent-specific data types, such as [`studyflow.csv`](/spec/studyflows.html) that describes the order the activities for the agent.
...
- Each agent has a folder named `agent_<ID>` where `<ID>` is a unique identifier for the human participant or computer agent. `<ID>` can be a number or a string, but must be unique within the dataset. BDM uses `agent_<ID>` for all datasets, whether the agents are humans, animals, or computational models (see [Naming things](/spec/conventions/naming.qmd)).
