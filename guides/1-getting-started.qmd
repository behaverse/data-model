...
Behaverse Data Model (BDM for short) is a dataset architecture that aims to provide interoperable data for modern cognitive projects. It is a set of conventions that help you organize behavioral data in a way that is easy to understand for both domain experts and technical experts. Whether you're an individual scientist or part of a larger lab, BDM can help you build a dataset that is easy to maintain and share with others.

BDM is designed to work with a wide range of cognitive tests and questionnaires. It is opinionated but designed to be flexible and extensible, so you can adapt it to your specific needs.
...
The `data/` folder contains the data files. Within this folder, data files are organized as `<AGENT>/<SESSION>/<ACTIVITY>/<DATA_FILE>`. This is commonly called [directory-based](https://arrow.apache.org/docs/python/generated/pyarrow.dataset.partitioning.html) [partitioning](https://en.wikipedia.org/wiki/Partition_(database)) and allows for easy access to the data files.

The `data/<AGENT>/` folder contains the data for a single human subject or computer agent, and the `data/<AGENT>/<SESSION>/` folder contains the data for a single session. Each session can contain multiple activities, and each activity can contain data files for several attempts. So the data folder is organized as Agent > Session > Activity > Data File.

Th `data/<AGENT>/` folder may also contain other agent-specific data types, such as [studyflows](../spec/general/3-studyflows.qmd).

Here is an example `data/` folder of three subjects, each with two sessions, and each session with two activities:
