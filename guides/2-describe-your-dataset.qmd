...
The machine-readable part relies primarily on the [front-matter](../spec/dataset-cards.qmd) of the file. Front-matters are a way to include metadata in a markdown file. They are written in YAML format and are placed at the top of the file between two sets of three dashes (`---`). For example:
...
The front-matter includes information about the dataset as a whole (such as the name, description, and creator) and the data (such as the variable names and data types). Here is an example:
  ...
  tables:
    - name: response
      tableSchema:
        columns:
          - name: "age"                                   # <1>
            description: "The age group of the agent"
            datatype:                                     # <2>
              base: "string"
              format: "18-25|26-35|36-45|46-55|56-"
            null: "NA"                                    # <3>
            notes:                                        # <4>
...
2. The type of the variable; here, a categorical variable defined by a format.
3. Missing data is represented by "NA".
4. Notes describing the possible values of the variable "age".

::: {.callout-tip appearance="simple"}
For small number of variables and values, you can describe the codebook in the `README.md` file (like above). But if it's too large to be included in the README file, the value of the `codebook` field can be a path to the codebook file. For example:

```yaml
codebook: codebook.yml
```
:::


::: {.callout-note appearance="simple"}
For a complete list of fields that can be included in the dataset card, see the [dataset card specification](/spec/dataset-cards.html).
:::
...
You can include the citation in the front-matter of the `README.md` file:
...
If you haven't already decided a license for your data, you can use services like [Choose a License](https://choosealicense.com/), [License Selector](https://ufal.github.io/public-license-selector/), or [Creative Commons license chooser](https://creativecommons.org/choose/). These tools helps you select a license that meets your needs and provides a human-readable summary of the license terms. Depending on your case, you can either put a name or URL in the `license` field (all are valid):
...
To summarize, dataset cards describe your dataset in a way that is easy to understand and useful. They include metadata about the dataset, such as the name, description, creator, and codebook, as well as information about the citation and license. By following these conventions, you can make your dataset more discoverable, interpretable, and reproducible.

The [dataset card specification](/spec/dataset-cards.qmd) lists every field, marks which are required, and shows a complete `README.md` combining the metadata, the codebook, and the human-readable description.
