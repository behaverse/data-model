...
2. keep the main information in one table (`trials.csv`) and delegate further details to supplementary tables (`stimuli.csv`, etc);
...
Within the Trial table (`trials.csv`), each row corresponds to a single trial. A trial is identified by a unique identifier (`trial_id`) and contains main information about the agent, the stimulus (i.e., the question), the options, the response, and the evaluation of the response. Here are the variables that are typically included in the Trial table for questionnaires:

:::{.callout-warning appearance="simple"}
## Multiple attempts
When a questionnaire is completed multiple times by the same agent in the same session, the data is stored in separate files per attempt and identified by a 1-indexed numerical suffix. For example, agent_2 in the example above completed questionnaire_3 twice; the data is stored in two files: `trials_1.csv` and `trials_2.csv`. The first file contains the data from the first attempt, and the second file contains the data from the second attempt. If there is only one attempt, the data would be stored in a single file named `trials.csv`.
