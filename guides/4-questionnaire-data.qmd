...
2. keep the main information in one table (`response_*.csv`) and delegate further details to supplementary tables (`stimulus_*.csv`, etc);
3. separate data into activity-specific files nested within agent-specific folders (e.g., `<AGENT>/<SESSION>/<ACTIVITY>/...`).

For example, here is a minimal dataset structure for a study with two agents completing three questionnaires (this study has a single session, so the optional session folder is omitted):
...
Each trial is a single interaction between an agent (e.g., a human participant) and a stimulus (e.g., a question). [Response table](../spec/trial/1-response.qmd) is a standalone table and contains the main information about the trial, while other supplementary tables may contain more specific information about [the agents](../spec/conventions/folder-structure.qmd), [stimuli](../spec/trial/2-stimulus.qmd) and [options](../spec/trial/3-option.qmd).
...
It is often necessary to evaluate the response provided by the agent. Importantly, how to evaluate a response depends on the specific question asked and often even depends on the questionnaire as a whole. The main features to evaluate a response within the Response table are the following:
