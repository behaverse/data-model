...
::: {.callout-note}
> It's OK to use plurals to name folders and files that hold collections, e.g., `instruments/`, `agents.csv`
:::
...
Furthermore, while we intend to conform to main existing conventions as much as possible, this is not always possible to do as different conventions and standards often conflict with each other. For example, [schema.org](https://schema.org) uses a "mixedCase" style to describe the elements of its schema (e.g., `countryOfOrigin`, `dateCreated`) while both the leading [python](https://pep8.org/) and [R](https://style.tidyverse.org/files.html) styleguides recommend using "lower_snake_case" (reusing the previous examples: `country_of_origin`, `date_created`.) The same is true for the file formats to use (e.g., `.csv` vs `.tsv`, `json-ld` vs `XML`.) and virtually all other aspects of data.
...
It is our view that the entity name in the data table should be generic to encompass a large set of use cases (e.g., children, animals, artificial agents). Unlike [BIDS](https://bids-specification.readthedocs.io/en/stable/), which uses "subject", we use "agent" in the data specification and avoid using any other term in the data (in particular, we never use "participants" in the data): "agent" covers human participants, non-human animals, and computational models alike. This does not mean however that human participants should be addressed as "agents" in general (for example, when communicating with them or about them); all participants are agents, but not all agents are participants (e.g., an AI agent). Naming a column or data file using "agent" is convenient and does not imply disrespect towards human study participants.
...
A reference to an agent can serve multiple purposes:

1. Ensure the uniqueness of an agent within a study; link all data from an agent within a study.

2. Select or identify data belonging to a particular agent. For example, if you see an outlier point in a plot and want to understand why that point occurred, it might be necessary to identify that data as belonging to a particular agent and then look in greater detail in that agent's data.

3. Use the agent's identifier to name their data files.

4. Ensure the uniqueness of an agent across studies. An agent may participate in multiple studies and it might be necessary (e.g., for meta-analyses) to determine whether agents participated in multiple studies.

These four purposes have somewhat different requirements.

Purpose 1 requires uniqueness within a study while .4 requires uniqueness in general. For purpose 1 it would suffice to have an agent counter and assign an integer to each agent within a study. This leads to shorter names which makes it practical for purposes 2 and 3. To ensure the uniqueness of agent identifiers in general requires longer strings, which are rather impractical for data analysis and management.

Consequently, we adopt the following conventions:

agent_index
: a string uniquely associated to each agent within a dataset. Similar to [BIDS](https://bids-specification.readthedocs.io/en/stable/02-common-principles.html#participant-names-and-other-labels) participant labels, this string is obtained by concatenating the string `agent_` with a zero-padded number which "counts" agents in a study (typically but not necessarily in chronological order). For example, the 15^th^ agent in a dataset might be assigned the agent_index value of agent_015.

The zero-padding width is chosen once per dataset, commensurate with the number of agents in the study (e.g., with more than 1000 agents, the 15^th^ agent would be `agent_0015`). By default, use three digits (e.g., `agent_015`)—enough for the vast majority of behavioral datasets. The same rule applies to any other indexed folder or label: session folders rarely need padding at all (`session_1`, `session_2`), and small illustrative examples may use unpadded numbers (e.g., `agent_1` in a three-agent example).

agent_id
: A string that uniquely identifies an agent (typically within a data collection institution or research group) and may be used to determine if the same agent participated in multiple studies wherein they might be assigned different agent_index values. agent_id may be created using custom procedure and look something like: `20025fe6-6868-47c6-a222-a5c06b49c8db`.

Note that `agent_id` need not appear in every data table; the *Agents* table (`agents.csv`) lists agent information, including their agent_index and agent_id values.
...
: we only use the suffix `_time` for `response_time`, which despite being ubiquitous in experimental psychology does not have a consensual definition (e.g., it may refer to the onset or offset of the response). In BDM, `response_time` is the duration between the earliest possible moment a response could have been completed and the moment it was actually completed (not the moment it was initiated).
...
- `response_time` is the duration between the earliest moment the response could have been completed (e.g., once the response options were available) and the moment it was completed;
