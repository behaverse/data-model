...
Note that such zero-padding may depend on the number of agents there are in a study (e.g., if there are less than 100, the first agent would be called "agent_01", but if there are more than 1000 agents in total, that agent would be called "agent_0001"). By default, we use a zero-padded string of length 4 as this will be enough for the vast majority of behavioral datasets.
