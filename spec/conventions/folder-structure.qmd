...
│   │           ├── option_1.csv
│   │           ├── events_1.ndjson                    # <12>
│   │           ├── mouse_1.ndjson.gz                  # <13>
│   │           └── mouse_1.ndjson.gz.timeseries.json  # <13>
│   └── agent_002/
│       └── ...
│
└── examples/                     # <14>
...
7. **Data**: Contains all the data collected within a study and that are within the scope of a dataset. Within `data/` there is one folder per agent that contains all the data about that agent.

8. **Agent data:** The agent data folder is labelled following the template `agent_{x}` where x is a zero-padded integer (e.g., `agent_001`, `agent_123`). The padding width follows the [naming conventions](naming.qmd): commensurate with the number of agents in the dataset, three digits by default.

9. **Studyflow**: Each agent data folder contains at its root a `studyflow.csv` file which explains what activities were completed by this agent (with a completion status), when they started and ended each activity and in what order the activities were completed.

10. **Sessions** (optional): In some cases it might be useful to group the data within each agent's folder into subfolders corresponding to multiple data collection sessions (e.g., `session_1/`, `session_2/`). This level of folders is optional and should be used when the data collection is divided into sessions.

11. **Activity data**: Additional folders are included within each agent's folder depending on the specific activities completed by that agent. For example, if the agent completed the BIS/BAS questionnaire and the Stroop test there would be two corresponding data folders (i.e., `bisbas`, `stroop`).

12. **Event stream** (optional): The raw events recorded during an attempt at the activity, stored as `events_<attempt>.ndjson` (optionally compressed to `.ndjson.gz`) — one event object per line, partitioned by attempt like `response_<attempt>.csv`. See [Raw acquisition data](#raw-acquisition-data-events-and-timeseries) below.

13. **Timeseries** (optional): Continuous signals (mouse trajectories, gaze, EEG, …) recorded during the activity: a payload file in a domain-appropriate format plus its metadata sidecar named `<payload-filename>.timeseries.json`, both in the activity folder.

14. **Examples** (optional): The `examples/` contains sample codes to showcase some aspects of the data (e.g., descriptive statistics, missing values plot).
...
We recommend using simple, short but clear names for data files so they are easier to type and can be fully displayed on screen. The location of a file provides additional context. For example `dataset/data/agent_001/session_1/response.csv` is preferred over `dataset/data/agent_001/agent-001_session-1_response.csv` as it is shorter and yet contains the same information.
