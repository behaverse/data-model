...
8. **Agent data:** The agent data folder is labelled following the template `agent_{x}` where x refers to a zero-padded integer (e.g., `agent_001`, `agent_123`); the number of zeros used for padding should be commensurate with the number of agents in the dataset.
...
10. **Sessions** (optional): In some cases it might be useful to group the data within each agent's folder into subfolders corresponding to multiple data collection sessions (e.g., `session_01/`, `session_02/`). This level of folders is optional and should be used when the data collection is divided into sessions.
