...
13. **Event stream** (optional): The raw events recorded during an attempt at the activity, stored as `events_<attempt>.ndjson` (optionally compressed to `.ndjson.gz`)—one event object per line, partitioned by attempt like `response_<attempt>.csv`. See [Raw acquisition data](#raw-acquisition-data-events-and-timeseries) below.
...
- **Event streams** live at `data/agent_{x}/[session_{n}/]<activity>/events_<attempt>.ndjson` (optionally `.ndjson.gz`), beside `response_<attempt>.csv`. The canonical at-rest form is NDJSON: one event object per line, each validating against the [event schema](https://behaverse.org/schemas/event). See the [Events data guide](/guides/7-events-data.qmd).
...
A recording engine produces one recording per run—that is, per agent, session, and attempt at an activity. Each part of that recording has one place in the layout:
...
| the derived trial data | the activity's CSVs: `response_<attempt>.csv` and its supporting tables (`stimulus_`, `option_`, `input_`, …) |
| the raw event stream | `events_<attempt>.ndjson[.gz]` beside those CSVs |
