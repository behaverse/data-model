...
## Response Table
 ...
 Main L1 table. Each row describes a response. More detailed information may be avaiable in other tables and link to this one via the primary key id.
...
id	[integer]
: The unique response identifier, generated in temporal order (meaning that a greater id refers to a response that occurred later in time). Unique within each table (i.e., each row in a table has a different id value.)
...
group_name	[string]
: Participants may be assigned to different groups (where typically, different groups will have different experiences within a study).
...
: Indexes in chronological order all responses emitted by a given subject within a study. 
...
: index of activity within subject.
...
: The name of the instrument used for collecting data (e.g., the name of the computer script used to run the test). Multiple versions of the same instrument may coexist.



instrument_id	[string]
: The unique id of the specific version of the instrument used for collecting data (e.g., the name of the computer script used to run the test). unique in the **Instrument** table.
...
: The more specific description of a job, which gives more information about what the participant sees and has to do. Whereas the `job_type` typically uses only verbs and adjectives, the `job_description` also contains nouns (e.g., “recall-forward-digits”, “recall-backward-letters”).
...
: Indexes in chronological (or spatial) order the stimuli shown within an instrument (counting one stimulus count per response). stimulus_index may for instance be used to refer to the nth question asked within a questionnaire.



stimulus_type	[enum]
: We distinguish the following stimulus types:
...
:::{.callout-note}
 ...
 While there is a stimulus_index_in_source to refer to the particular stimulus that was used, we don’t have an equivalent opiton_index_in_source since all options are displayed. Instead, we use response_index and expected_response_option_index to refer to a particular option within the set of options. 
...
:::{.callout-note}
 ...
 response_option_index refers to an entry in the Option table (i.e., there is no  Response table). 
