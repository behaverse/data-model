...
def render_table_page(schema: str, table: dict, ref_base: str, index: int | None = None) -> str:
    name = table["name"]
    ...
    for note in table.get("notes") or []:
        level, text = _split_note_level(note)
        kind = {"warning": "warning", "important": "important", "tip": "tip"}.get(level, "note")
        out += [f"::: {{.callout-{kind} appearance=\"simple\"}}", text, ":::", ""]
    ...
    if "tables" in doc:  # multi-table schema (trial)
        ...
        for i, table in enumerate(doc["tables"], 1):
            (out_dir / f"{_table_slug(i, table['name'])}.qmd").write_text(
                render_table_page(schema, table, ref_base, i))
...
def _vocab_entries(vocab: dict) -> list[dict]:
    ...
    for c in vocab["concepts"]:
        ...
        if c.get("notes"):
            e["notes"] = [rewrite_legacy_links(_note_plain(n)) for n in c["notes"]]
        entries.append(e)
    return entries
