...
def render_table_page(schema: str, table: dict, index: int | None = None) -> str:
    ...
    if index is not None:
        # The filename is the published slug — stable across upstream reorders — so the
        # artifact's table order reaches the sidebar as front matter instead. The numbered
        # URLs the site used to publish stay alive as aliases (unpadded and zero-padded).
        out.append(f"order: {index}")
        old = {f"/spec/{schema}/{index}-{name.lower()}.html",
               f"/spec/{schema}/{index:02d}-{name.lower()}.html"}
        out.append(f'aliases: {json.dumps(sorted(old))}')
...
def render_index_page(schema: str, doc: dict, ref_base: str,
    ...
    for t in doc["tables"]:
        out.append(f"| [{t['name']}]({t['slug']}.qmd) "
...
def generate_schema_pages(schema: str, doc: dict, base_url: str,
    ...
    if "tables" in doc:  # multi-table schema (trial, studyflow, timeseries)
        ...
        for i, table in enumerate(doc["tables"], 1):
            (out_dir / f"{table['slug']}.qmd").write_text(
