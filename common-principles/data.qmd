...
source data is not useable: they can be messy, come in diverse, sometimes proprietary formats, and lack documentation.
source data may contain personal data the data sharer is not aware of (e.g., participants’ full name, IP address); by defining what data is to be shared and extracting only that data it seems that such accidental privacy breaches could be avoided.
the extraction of data from the source data into usable data is outside the scope of responsibility of the data analyst.
...
In short then, we say “don’t share source data”, instead share well documented, well formatted raw data.
