...
It seems impossible to have everyone agree on what the best solution is—fields have different traditions and conventions, people have different preferences, etc.
...
The implication is that it is impossible to make everyone happy. When chosing among different alternatives we try to follow the following principles:
  ...
  b) if there are good technical reasons to favor one alternative over another. This is the case for example when chosing between json-ld and xml or when chosing between tsv and csv. The difficulty here is that the community might be more used to one kind of file format versus another—but hey, it might be time to evolve;-)
